        ...
        void OnGUI()
        {
            ...
            if (!manager.IsClientConnected() && !NetworkServer.active && manager.matchMaker == null)
            {
                if (noConnection)
                {
                    if (UnityEngine.Application.platform != RuntimePlatform.WebGLPlayer)
                    {
                        if (GUI.Button(new Rect(xpos, ypos, 400, 60), "LAN Host(H)"))
                        {
                            manager.StartHost();
                        }
                        ypos += spacing;
                    ...
                    manager.networkAddress = GUI.TextField(new Rect(xpos + 210, ypos, 190, 60), manager.networkAddress);
                    ypos += spacing;
            ...
            if (!NetworkServer.active && !manager.IsClientConnected() && noConnection)
            {
                ...
                else
                {
                    ...
                    GUI.Label(new Rect(xpos, ypos, 400, 40), "MM Uri: " + manager.matchMaker.baseUri);
                    ypos += spacing;
